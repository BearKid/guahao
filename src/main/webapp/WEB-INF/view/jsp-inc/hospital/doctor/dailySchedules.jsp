...
<table id="doctorDailyScheduleList">
    ...
    <c:if test="${not empty doctorDailySchedulePaging.items}">
        <c:forEach var="doctorDailySchedule" items="${doctorDailySchedulePaging.items}">
            <tr>
                <td colspan="5">
                    <form id="doctorDailyScheduleSaveForm-${doctorDailySchedule.id}" class="doctorDailyScheduleSaveForm"
                          action="${applicationScope.contextPath}/hospital/dailySchedule/saveOrUpdate" method="POST">
                            ...
                            <tr>
                                ...
                                <c:choose>
                                    <c:when test="${empty doctorDailySchedule.doctorPerTimeScheduleList}">
                                        ...
                                        <td>
                                            <a href="#" class="doctorPerTimeScheduleInputAdd" data-doctor-daily-schedule-id="${doctorDailySchedule.id}">新增</a>
                                    ...
                                    <c:otherwise>
                                        ...
                                        <c:if test="${doctorDailySchedule.totalSource != doctorDailySchedule.oddSource}">
                                            <td>
                                                <a href="#" data-doctor-daily-schedule-id="${doctorDailySchedule.id}">展开</a>
                                            </td>
                                        </c:if>
                        ...
                        <table id="doctorPerTimeScheduleList-${doctorDailySchedule.id}"
                               class="doctorPerTimeScheduleList">
                            <c:forEach var="doctorPerTimeSchedule"
                                ...
                                <tr class="doctorPerTimeSchedule" id="doctorPerTimeSchedule-${doctorPerTimeSchedule.id}"
                                    ...
                                    style="display:none;">
                                    <td width="15%">${doctorPerTimeSchedule.startTime} ~ ${doctorPerTimeSchedule.endTime}</td>
                        ...
                        <!--某天详细排班-修改-->
                        <table id="doctorPerTimeScheduleInputList-${doctorDailySchedule.id}" class="doctorPerTimeScheduleInputList">
                            <c:forEach var="doctorPerTimeSchedule"
                                       items="${doctorDailySchedule.doctorPerTimeScheduleList}">
                                <tr class="doctorPerTimeScheduleInput"
                                    id="doctorPerTimeScheduleInput-${doctorPerTimeSchedule.id}"
                                    data-id="${doctorPerTimeSchedule.id}" style="display:none;">
                                    ...
                                    <td width="15%"><input name="totalSource" type="text"
                                               value="${doctorPerTimeSchedule.totalSource}"/>
                                    </td>
                                    <td width="15%">-</td>
                                    <td width="15%">-</td>
                                    <td>
...
<script>
    ...
    $(".briefDoctorDailySchedule .doctorPerTimeScheduleInputAdd").click(function () {
        ...
        /*切换挂号费字段为输入框*/
        var priceInput = briefDoctorDailyShchedule.find('input');
        priceInput.show();
    ...
    $(".doctorPerTimeScheduleDelete").click(function () {
        ...
        var curDoctorPerTimeScheduleId = $curDoctorPerTimeScheduleInput.attr("id");
        ...
        if (curDoctorPerTimeScheduleId) {//数据库记录删除
            var url = "${applicationScope.contextPath}/hospital/doctor/${doctorDailyScheduleQo.doctorId}/dcotorPerTimeSchedule/" + curDoctorPerTimeScheduleId;
    ...
    $(".doctorDailyScheduleSave").click(function () {
        var $this = $(this);
        var doctorDailyScheduleId = $this.data("doctorDailyScheduleId");
//        var doctorPerTimeScheduleInputList = $("#doctorPerTimeScheduleList-" + doctorDailyScheduleId);
//        var doctorPerTimeScheduleInputs = doctorPerTimeScheduleInputList.find(".doctorPerTimeScheduleInput");
...
//                    $("#jsSubmitdoctorSearchForm").trigger("click");
                }, 2000);
            }
        });
    });
